 ...
 * test user's credit balance without real billing, since there's no signup/
 * payment flow yet to grant credits any other way (see backend/README.md's
 * Pricing & Credits — CreditsService.grantMonthlyCredits/credit are otherwise
 * only ever called internally). Returns 404 in production — as if the route
 * doesn't exist at all — rather than 403, so this never even hints at its own
